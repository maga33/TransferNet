...
SelectSrcTgkSegBinaryLayer<Dtype>::~SelectSrcTgkSegBinaryLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void SelectSrcTgkSegBinaryLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(transform_param.has_mean_file() == false) << 
         "SelectSrcTgkSegBinaryLayer does not support mean file";
  ...
  while (std::getline(infile, linestr)) {
    ...
    item.use_seg_label = true;
    if((iss.rdstate() & std::ifstream::eofbit) == 0) {
      int use_seg_label;
      iss >> use_seg_label;
      item.use_seg_label = use_seg_label > 0;
      CHECK_EQ(use_seg_label, 1);
    }
  ...
  LOG(INFO) << "A total of " << src_lines_.size() << " source images.";
  LOG(INFO) << "A total of " << tgk_lines_.size() << " target images.";
  LOG(INFO) << "A total of " << seg_count << " target segmentation annotations.";
...
template <typename Dtype>
void SelectSrcTgkSegBinaryLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void SelectSrcTgkSegBinaryLayer<Dtype>::load_batch(Batch_seg<Dtype>* batch) {
...
//    LOG(INFO) << "Load batches from source images";
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for (int i = 0; i < pixel_count; i++) {
      if ((*lines_)[*lines_id_].use_seg_label) {
...
INSTANTIATE_CLASS(SelectSrcTgkSegBinaryLayer);
REGISTER_LAYER_CLASS(SelectSrcTgkSegBinary);
}  // namespace caffe
