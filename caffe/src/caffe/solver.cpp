...
void Solver<Dtype>::InitTrainNet() {
  ...
  // Added by Jun ------------------------
  if (param_.has_encoder_net()) {
    parallel_encoder_.reset(new ParallelEncoder<Dtype>(net_,
        param_.encoder_net(), TRAIN, param_.encoder_device_id()));
  }
  //--------------------------------------
}
...
void Solver<Dtype>::InitTestNets() {
  ...
  for (int i = 0; i < num_test_net_instances; ++i) {
    ...
    // Added by Jun ------------------------
    if (param_.has_encoder_net()) {
      shared_ptr<ParallelEncoder<Dtype> > test_encoder;
      test_encoder.reset(new ParallelEncoder<Dtype>(test_nets_[i],
          param_.encoder_net(), TEST, param_.encoder_device_id()));
      parallel_test_encoders_.push_back(test_encoder);
    }
    //--------------------------------------
  }
}
...
void Solver<Dtype>::Step(int iters) {
  ...
  if (parallel_encoder_) {
    parallel_encoder_->StartInternalThread();
  }
  //--------------------------------------
...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  // Added by Jun ------------------------
  if (parallel_test_encoders_.size()) {
    parallel_test_encoders_[test_net_id]->StartInternalThread();
  }
  //--------------------------------------
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    // Added by Jun ------------------------
    if (parallel_test_encoders_.size()) {
      parallel_test_encoders_[test_net_id]->Sync();
      if (i < param_.test_iter(test_net_id) - 1) {
        parallel_test_encoders_[test_net_id]->StartInternalThread();
      }
    }
    //--------------------------------------
